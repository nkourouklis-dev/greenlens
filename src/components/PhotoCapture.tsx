  ...
  return (
    <div className="fixed inset-0 z-30 flex flex-col bg-[#0e1411] text-white">
      ...
      <div className="relative min-h-0 flex-1 overflow-hidden">
        ...
        <div
          aria-hidden
          className="pointer-events-none absolute inset-x-0 top-0 h-40 bg-black/45"
        />

        {/* Top controls: back, step, barcode. */}
        <div className="absolute inset-x-0 top-0 z-10 flex flex-col gap-3 px-4 pt-[max(0.75rem,env(safe-area-inset-top))]">
        ...
        <div
          ref={guideRef}
          className={`absolute inset-x-6 bottom-20 top-[calc(max(0.75rem,env(safe-area-inset-top))+7.5rem)] ${previewUrl ? "invisible" : ""}`}
        >
          <GuideCorners />
        ...
        {!previewUrl && isCameraActive && (
          <div className="pointer-events-none absolute inset-x-0 bottom-9 flex justify-center px-4">
            <p
              aria-live="polite"
              className="rounded-full bg-black/65 px-3.5 py-2 text-center text-[13px] font-medium"
            >
              {seemsTooFar
        ...
        {previewUrl && (
          <div className="absolute inset-x-4 bottom-10 top-[calc(max(0.75rem,env(safe-area-inset-top))+7.5rem)]">
            <img
              src={previewUrl}
              alt={title}
      ...
      {/* Bottom sheet. */}
      <div className="relative -mt-6 rounded-t-[28px] bg-canvas px-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-5 text-ink">
            ...
            <>
              ...
              <div className="flex flex-col gap-2.5">
                <button
                  ...
                  className="h-14 w-full rounded-2xl bg-accent px-5 text-base font-bold text-on-accent transition active:scale-[0.99] disabled:cursor-not-allowed disabled:bg-surface-muted disabled:text-ink-faint"
                >
              ...
              {tips.length > 0 && (
                <ul className="flex flex-wrap gap-2">
                  {tips.map(({ label, Icon }) => (
                    <li
                      key={label}
                      className="flex items-center gap-1.5 rounded-lg bg-emerald-950 px-2.5 py-1.5 text-xs font-semibold text-emerald-200"
                    >
              ...
              {isCameraActive ? (
                <div className="flex items-center justify-between px-2">
                  <button
                    ...
                    className="flex h-13 w-13 items-center justify-center rounded-2xl border border-line bg-surface text-ink transition active:scale-95"
                  >
                    <ImageIcon size={22} />
                  </button>
                  ...
                  <button
                    ...
                    className="flex h-20 w-20 items-center justify-center rounded-full border-4 border-accent p-1.5 transition active:scale-95 disabled:opacity-60"
                  >
                    <span className="h-full w-full rounded-full bg-accent" />
                  </button>

                  {/* Keeps the shutter centred against the gallery button. */}
                  <span aria-hidden className="h-13 w-13" />
                </div>
              ) : (
