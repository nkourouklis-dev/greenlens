...
Bonus +5 για μηδέν επισημασμένα πρόσθετα (E-numbers)
```

Τα 14 επίσημα αλλεργιογόνα της ΕΕ (γλουτένη, γάλα, αυγό, θειώδη, ξηροί
καρποί, σουσάμι, σόγια κ.λπ.) **δεν** αφαιρούν πόντους όταν η μόνη
επισήμανση είναι ότι πρόκειται για γνωστό αλλεργιογόνο. Εμφανίζονται σε ένα
ενιαίο info banner (`allergenNotice`, βλ. `worker/allergens.ts`) και ισχύει
το ίδιο και στο ingredients και στο nutrition path. Αν η επισήμανση αναφέρει
πραγματικό πρόβλημα (τεχνητό πρόσθετο, μη δηλωμένη ποσότητα, τοξικότητα), η
αφαίρεση παραμένει κανονικά.
