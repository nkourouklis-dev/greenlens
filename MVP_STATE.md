...
- **Fragrance allergens δεν έχουν "declaration-only" υποβάθμιση.** Αν το AI
  ...
  ασυνέπεια στη λογική που αξίζει να λυθεί. *(13/9: στην κανονική διαδρομή
  λύθηκε με το `28d17f9` — οι αφαιρέσεις βγαίνουν από κανόνες, και parfum +
  τα αλλεργιογόνα αρωμάτων είναι μία ομάδα `fragrance_allergen` που χρεώνεται
  μία φορά (5–6 πόντοι), ενώ τα 14 food allergens δεν χρεώνονται. Στην
  εφεδρική διαδρομή χωρίς κανόνες, τα αρώματα χρεώνονται πλέον επίσης μία
  φορά — `worker/fragranceFallback.test.ts`.)*
